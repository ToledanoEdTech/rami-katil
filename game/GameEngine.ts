...
export class GameEngine {
  ...
  constructor(
    ...
    callbacks: { onStatsUpdate: any, onGameOver: any, onFeedback: any, onAchievement: any }
  ) {
    this.canvas = canvas;
  ...
  startRound() {
    ...
    if (this.level % 10 === 0 && !this.boss) { this.startBossFight(); return; }
  ...
  updateEnemies() {
      ...
      for (let i = 0; i < this.enemyPool.length; i++) {
          ...
          for (let j = 0; j < this.projectilePool.length; j++) {
              ...
              if (p.type === 'beam' ? (Math.abs(p.x - e.x) < beamWidth && e.y < p.y) : (Math.hypot(p.x - e.x, p.y - e.y) < (p.type === 'fire' ? e.radius*2.5 : e.radius*hitTolerance))) {
                  hit = true; if (p.type !== 'beam') p.active = false; break;
              }
          }
  ...
  handleHit(isCorrect: boolean, x: number, y: number) {
      if (isCorrect) {
          ...
          this.onStatsUpdate({ score: this.score, combo: this.combo, level: this.level });
      } else {
  ...
  draw() {
    ...
    if (!this.playerExploding) {
      ...
      this.ctx.fillText(this.config.sugiaTitle || '', this.width/2, this.height - 20);
    }
  }
  ...
  renderShip() {
      ...
      if (skin === 'skin_choshen') {
          ...
          this.ctx.beginPath(); this.ctx.roundRect(-w/2, -h/2, w, h, 6); this.ctx.fill(); this.ctx.stroke();
          this.ctx.strokeStyle = 'rgba(0,0,0,0.1)'; this.ctx.strokeRect(-w/2 + 3, -h/2 + 3, w - 6, h - 6);
          const colors = ['#ef4444', '#3b82f6', '#22c55e', '#a855f7', '#f97316', '#06b6d4', '#ec4899', '#84cc16', '#64748b', '#eab308', '#d946ef', '#f43f5e'];
          const gemSize = 10; const paddingX = 14, paddingY = 14;
          for(let row=0; row<4; row++) { for(let col=0; col<3; col++) {
              const idx = row * 3 + col; const gx = -w/2 + paddingX + col * (w - 2*paddingX)/2; const gy = -h/2 + paddingY + row * (h - 2*paddingY)/3;
              this.ctx.shadowBlur = 5; this.ctx.shadowColor = colors[idx]; this.ctx.fillStyle = colors[idx];
              this.ctx.beginPath(); this.ctx.arc(gx, gy, gemSize/2, 0, Math.PI*2); this.ctx.fill();
              this.ctx.fillStyle = 'rgba(255,255,255,0.4)'; this.ctx.beginPath(); this.ctx.arc(gx - 2, gy - 2, 1.5, 0, Math.PI*2); this.ctx.fill();
      ...
      if (skin === 'skin_torah') {
          ...
          this.ctx.fillStyle = '#b45309'; this.ctx.strokeStyle = '#78350f'; this.ctx.lineWidth = 2;
          this.ctx.beginPath(); this.ctx.roundRect(-25, -35, 10, 70, 4); this.ctx.fill(); this.ctx.stroke();
          this.ctx.beginPath(); this.ctx.roundRect(15, -35, 10, 70, 4); this.ctx.fill(); this.ctx.stroke();
          this.ctx.fillStyle = '#fbbf24';
          this.ctx.beginPath(); this.ctx.arc(-20, -35, 6, 0, Math.PI*2); this.ctx.fill();
          this.ctx.beginPath(); this.ctx.arc(-20, 35, 6, 0, Math.PI*2); this.ctx.fill();
          this.ctx.beginPath(); this.ctx.arc(20, -35, 6, 0, Math.PI*2); this.ctx.fill();
          this.ctx.beginPath(); this.ctx.arc(20, 35, 6, 0, Math.PI*2); this.ctx.fill();
          this.ctx.fillStyle = '#fef3c7'; this.ctx.beginPath(); this.ctx.roundRect(-15, -25, 30, 50, 2); this.ctx.fill();
          this.ctx.strokeStyle = 'rgba(0,0,0,0.1)'; this.ctx.lineWidth = 1;
          for(let i=0; i<5; i++) { this.ctx.beginPath(); this.ctx.moveTo(-10, -15+i*8); this.ctx.lineTo(10, -15+i*8); this.ctx.stroke(); }
          this.ctx.fillStyle = 'rgba(251, 191, 36, 0.2)'; this.ctx.beginPath(); this.ctx.arc(0, 0, 35 + pulse, 0, Math.PI*2); this.ctx.fill();
          this.ctx.restore(); return;
      }
      ...
      this.ctx.beginPath(); this.ctx.moveTo(10, -5); this.ctx.lineTo(50, 25); this.ctx.lineTo(50, 40); this.ctx.lineTo(10, 25); this.ctx.closePath(); this.ctx.fill();
      const fG = this.ctx.createLinearGradient(-25, 0, 25, 0); fG.addColorStop(0, secondary); fG.addColorStop(0.5, primary); fG.addColorStop(1, secondary);
      ...
      const cG = this.ctx.createLinearGradient(0, -35, 0, -5); cG.addColorStop(0, '#fff'); cG.addColorStop(0.3, cockpit); cG.addColorStop(1, secondary);
